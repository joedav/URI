 ...
 * pode-se obter distância percorrida e, em seguida,
 ...
 * 
 * Trocar "prompt()" por "lines.shift()" quando passar para o URI
 */

const tempoGastoHoras = parseInt(prompt());
const velocidadeMediaKM = parseInt(prompt());

function gastoDeCombustivel(tempoH, veloMediaKM) {
  const kmL = 12;

  // (tempo gasto vezes a velocidade media) dividido pela quantidade de km por litro
  const litros = (tempoH * veloMediaKM) / kmL;

  console.log(litros.toFixed(3));
}

gastoDeCombustivel(10, 85);
gastoDeCombustivel(2, 92);
gastoDeCombustivel(22, 67);