...
private fun gastoDeCombustivel() {
    val tempoGastoEmHoras: Int? = readLine()?.toIntOrNull()
    val velocidadeMediaGastaEmKM: Int? = readLine()?.toIntOrNull()
    if (tempoGastoEmHoras != null && velocidadeMediaGastaEmKM != null) {
        val litrosNecessariosParaViagem: Double = calculaListrosNecessariosParaViagem(tempoGastoEmHoras, velocidadeMediaGastaEmKM)
        println("%.3f".format(litrosNecessariosParaViagem))
    }
}

private fun calculaListrosNecessariosParaViagem(tempoGastoEmHoras: Int, velocidadeMediaGastaEmKM: Int): Double {
    return (tempoGastoEmHoras * velocidadeMediaGastaEmKM) / 12.0
}
